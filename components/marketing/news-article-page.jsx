...
export function NewsArticlePage({ post }) {
  return (
    <>
      ...
      <article>
        <header style={{ position: "relative", overflow: "hidden", borderBottom: "1px solid var(--border-1)", background: "var(--bg-0)" }}>
          ...
          <div style={{ position: "relative", maxWidth: 1040, margin: "0 auto", padding: "82px 32px 56px" }}>
            <Link href={routes.news} className="btn ghost sm" style={{ marginBottom: 28 }}>
              <Icon d={I.chevRight} size={12} style={{ transform: "rotate(180deg)" }} /> News
            </Link>
          ...
          <div className="news-article-grid">
            ...
            <aside className="news-article-aside">
              <NewsletterSignup
                ...
                heading="Get the next log."
                sub="We are publishing product notes, build videos, and launch access updates as Geek Night Engine comes together."
                compact
              />
      ...
      <CtaStrip heading="Follow the next build." sub="We will share the first public demo video and product notes through the launch newsletter." cta="Subscribe for build logs" />
      <SiteFooter />
    </>
  );
}
